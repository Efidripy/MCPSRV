...
- Дополнительно, установщик теперь сам подстраховывается: если в Dockerfile внезапно найдена legacy-строка `python3 -m pip install -U pip`, он собирает образ по временно “очищенному” Dockerfile.
- Если сборка всё равно упала с `externally-managed-environment`, установщик автоматически делает повторную попытку сборки через принудительно санитизированный Dockerfile.
- Если вы не видите свежие изменения на GitHub, проверьте, что пушите актуальную ветку (например, `work`) и создаёте PR из неё в `main`.
...
- Каждое изменение делайте отдельным PR (новый PR на каждый набор правок).
- Рекомендуемый поток: `work` -> PR в `main`.
- После каждого нового коммита в `work` проверяйте, что PR обновился или создайте новый PR, если нужен отдельный change-set.
